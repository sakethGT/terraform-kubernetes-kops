# Dedicated EBS volumes for etcd: one set for the main cluster state and one
# for the events stream. Using separate volumes allows independent IOPS tuning
# and prevents event churn from impacting cluster state performance.
